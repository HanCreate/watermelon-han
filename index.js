(() => {
  const Engine = Matter.Engine,
      LEngine = Matter.Engine,
      Render = Matter.Render,
      LRender = Matter.Render,
      World = Matter.World,
      LWorld = Matter.World,
      Body = Matter.Body,
      Bodies = Matter.Bodies,
      lBodies = Matter.Bodies,
      Events = Matter.Events,
      Composite = Matter.Composite;

  const parent = document.getElementById("game");
  const lparent = document.getElementById("nextList");
  ...
  const lctx = nextCanvas.getContext("2d");

  const engine = Engine.create();
  const lengine = LEngine.create();
  ...
  const lbackground = lBodies.rectangle(240, 360, 480, 720, {
    isStatic: true,
    render: { fillStyle: "#ff4081" },
  });
  lbackground.collisionFilter = {
    group: 0,
    category: 1,
    mask: -2,
  };
  const lground = lBodies.rectangle(400, 1220, 810, 1000, {
    isStatic: true,
    render: { fillStyle: "transparent" },
  });
  const lwallLeft = lBodies.rectangle(-100, 500, 100, 1000, {
    isStatic: true,
    render: { fillStyle: "transparent" },
  });
  const lwallRight = lBodies.rectangle(730, 500, 100, 1000, {
    isStatic: true,
    render: { fillStyle: "transparent" },
  });
  World.add(engine.world, [wallLeft, wallRight, ground, background]);
  LWorld.add(lengine.world, [lwallLeft, lwallRight, lground, lbackground]);
  ...
  addEventListener("click", () => {
    ...
    if (ball != null) {
      ...
      console.log("1 : "+ nextSize);
      //LWorld.remove(lengine.world, nextSize[0]);
  ...
  Events.on(engine, "beforeUpdate", () => {
    ...
    for (let i = 4; i < bodies.length; i++) {
      ...
      if (body.position.y < 100) {
        if (
            body !== ball && body !== nball && nball !== null &&
  ...
  Events.on(lengine, "beforeUpdate", () => {
    isLineEnable = false;
  });
  ...
  function collisionEvent(e) {
    ...
    e.pairs.forEach((collision) => {
      ...
      if (bodies[0].size === undefined || bodies[1].size === undefined  && (bodies[0] !== nball && bodies[1] !== nball && nball !== null) ) return;
  ...
  function createNextBall(size) {
    if (nball !== null){
      LWorld.remove(lengine.world, nball);
    }
    nball = nextBall(lrender.options.width / 2, 50, size);

    LWorld.add(lengine.world, nball);
  }

  function nextBall(x, y, size) {
    d = lBodies.circle(x, y, size * 10, {
      render: {
        sprite: {
          texture: `assets/img/${size}.png`,
          xScale: size / 12.75,
